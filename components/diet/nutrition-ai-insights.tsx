...
import { generateNutritionRecommendations } from "@/actions/ai-actions";
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${cfg.badgeClass}`}>
              {insight.type}
            </span>
          </div>
...
export function NutritionAIInsights() {
  ...
  function handleGenerate() {
    ...
    startTransition(async () => {
      const result = await generateNutritionRecommendations();
      if (result.success) {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">AI Nutrition Tips</h2>
        ...
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-1.5 rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-amber-700 disabled:opacity-60"
        >
          {isPending ? (
            <>
            ...
            <>
              ...
              {insights ? "Regenerate" : "Get Nutrition Tips"}
            </>
          )}
        </button>
      </div>
      ...
      {!isPending && !error && !insights && (
        <div className="rounded-xl border border-dashed border-border p-10 text-center">
          <Sparkles className="mx-auto mb-3 size-6 text-muted-foreground/40" />
          <p className="text-sm font-medium">No tips yet</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Click "Get Nutrition Tips" for personalized advice based on your last 14 days of diet logs.
          </p>
        </div>
      )}
